                ...
                <div class="main_content">
                    ...
                    <Form :model="postForm" label-position="right" :label-width="80" :rules="ruleValidate">
                        ...
                        <FormItem label="添加标签" prop="tags">
                            <Select v-model="postForm.tags" filterable multiple allow-create :max-tag-count="6"
                                    @on-create="selectPostTags" placeholder="按回车键Enter创建标签">
                                <template #prefix>
                        ...
                        <FormItem>
                            <Button type="primary" @click="updatePost" :loading="loading">立即更新</Button>
            ...
            onMounted(() => {
                ...
                getPostDetail(id).then(res => {
                    if (res.code === 200) {
                        ...
                        postForm.title = res.result.title;
                        postForm.content = res.result.content;
                        postForm.sectionId = [res.result.parentSectionId, res.result.sectionId];
                        postForm.tags = res.result.tags;
                        if (postForm.tags && postForm.tags.length > 0) {
                            postForm.tags.forEach(e => {
                                selectPostTags(e);
                            })
                        }
                    } else {
